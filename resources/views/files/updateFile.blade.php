<!-- Update file -->
<form action="/files/{{$file->id}}/updateFile" method="POST" enctype="multipart/form-data">
  {{ csrf_field() }}

  <!-- Modal Header -->
  <div class="modal fade" id="fileEditModal{{md5($file->id)}}" tabindex="-1" role="dialog" aria-labelledby="fileModalLabel{{md5($file->id)}}" aria-hidden="true">
    <div class="modal-dialog" role="dialog">
      <div class="modal-content" style="background: #1e2129">
        <div class="modal-header">
          <h5 class="modal-title" id="fileModalLabel{{md5($file->id)}}" style="color: #cc6600;">Update file:  <span class="text-muted">{{ str_replace('_', ' ', $file->name).'.'.$file->type }}</span></h5>
